...
impl IntConfigAccel {
    ...
    fn int_gen_cfg_xl(&self) -> u8 {
        ...
        if self.interrupt_zaxis_high.status() {
            data |= 1 << 5;
        }
        if self.interrupt_zaxis_low.status() {
            data |= 1 << 4;
        }
        if self.interrupt_yaxis_high.status() {
            data |= 1 << 3;
        }
        if self.interrupt_yaxis_low.status() {
            data |= 1 << 2;
        }
        if self.interrupt_xaxis_high.status() {
            data |= 1 << 1;
        }
        if self.interrupt_xaxis_low.status() {
            data |= 1;
        }
        data
    }
    ...
    T: Interface,
{
    ...
    pub fn accel_int_latching(&mut self, setting: INT_LATCH) -> Result<(), T::Error> {
        let reg_value =
            self.read_register(Sensor::Accelerometer, register::AG::CTRL_REG4.addr())?;
        ...
        self.interface
            .write(Sensor::Gyro, register::AG::CTRL_REG4.addr(), data)?;
